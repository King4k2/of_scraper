...
def upload_stories(ofs_data_dict: dict, account_name: str, people_tag: str):
    ...
    service = ChromeService(executable_path='<путь к драйверу>')
    # disable webdriver mode
    options.add_argument("start-maximized")
    options.add_experimental_option("excludeSwitches", ["enable-automation"])
    options.add_argument("--disable-blink-features=AutomationControlled")
    options.add_argument(f"user-agent={ofs_data_dict.get(account_name)[0]}")
    options.add_argument(f"accept-language={ofs_data_dict.get(account_name)[1]}")
